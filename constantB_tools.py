...
@jax.jit
def _residual(B, K):
    """F(B) = (div B, (|B|^2-1)/2).  NOTE: the second entry is evaluated
    pointwise and is blind to aliasing -- see `refine` for the honest
    check."""
...
@partial(jax.jit, static_argnames=('pcg', 'weighted', 'verbose'), donate_argnums=(0,))
def _gn_jit(B, K, K2, Wm2, sweeps, cgit, tol, pcg, weighted, verbose):
    ...
    init = (B, jnp.array(0), jnp.asarray(jnp.inf), jnp.array(0), jnp.array(False))
    Bf, _kf, res_last, cif, donef = lax.while_loop(sweep_cond, sweep_body, init)

    r1f, r2f = _residual(Bf, K)
...
class Solver:
    ...
    def residual(self, B):
        ...
        pointwise and is blind to aliasing -- see `refine` for the honest
        check."""
        return _residual(jnp.asarray(B), self.K)
    ...
    def gn(self, B, sweeps=6, cgit=500, tol=1e-10, verbose=False, pcg=False):
        ...
        save_state)."""
        B = jnp.asarray(B)
        Bf, res, ci = _gn_jit(B, self.K, self.K2, self._Wm2,
...
def main():
    ...
    if args.cmd == 'series':
        ...
        B, res, ci = Solver(args.grid).gn(B, sweeps=6, cgit=args.cgit, pcg=args.pcg)
        ...
        B, eps, meta = load_state(args.state)
        B, res, ci = Solver(B.shape[1:]).gn(B, sweeps=args.sweeps,
